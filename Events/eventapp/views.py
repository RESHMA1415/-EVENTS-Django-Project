...
def event_booking(request):
    if 'uname' in request.session:
        ...
        return render(request,'events_booking.html',context=data)
    else:
        data = {'status':'You need to login first'}
        return render(request,'registration.html',context=data)
...
def db_events_booking(request):
    if request.method == 'POST':
        ...
        try:
            book = Book_events.objects.get(date=date)
            ...
            book = Book_events(userid=user.userid,name=user.name,mobile=mobile,date=date,time=time)
            book.save()
